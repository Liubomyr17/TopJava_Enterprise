...
/*
    private static List<MealTo> getFilteredByAtomic(List<Meal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
            ...
            Map<LocalDate, AtomicBoolean> exeededMap = new HashMap<>();

            List<MealTo> mealsTo = new ArrayList<>();
            meals.forEach(meal -> {
                AtomicBoolean wrapBoolean = exeededMap.computeIfAbsent(meal.getDate(), date -> new AtomicBoolean());
                ...
                if (TimeUtil.isBetween(meal.getTime(), startTime, endTime)) {
                    mealsTo.add(createTo(meal, wrapBoolean));
                }
            });
            return mealsTo;
    }
